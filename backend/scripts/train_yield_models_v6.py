...
log = logging.getLogger("train_yield_v6")

# v6 = real-only-trained production models. Same parquet input as v5
...
METRICS_OUT = ROOT / "data" / "processed" / "model_metrics_v6.json"
...
def _split_chronological(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """v6 chronological split — adapted to real-data availability.

    v5 used train ≤2021 / val 2022 / test 2023, but 2022 + 2023 are
    synthetic (no Держстат XLSX for those years yet). v6 trains and
    tests entirely on real years:
      train: 2018-2019 (real)
      val:   2020 (real)
      test:  2021 (real)
    """
    train = df[df["year"].isin(V6_TRAIN_YEARS)]
    val = df[df["year"] == V6_VAL_YEAR]
    test = df[df["year"] == V6_TEST_YEAR]
...
def main() -> int:
    ...
    # v6: filter to is_real_yield=True only — no synthetic confound.
    if "is_real_yield" not in df.columns:
        log.error("v6 requires 'is_real_yield' column in parquet — run "
    ...
    log.info("v6 chronological split: train=%s val=%d test=%d",
             list(V6_TRAIN_YEARS), V6_VAL_YEAR, V6_TEST_YEAR)
